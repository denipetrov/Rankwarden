...
export class MplusCharacterAccumulator {
  ...
  /** The finished documents, with `mythicScore` summed over the best runs. */
  drain(): MplusCharacterDocument[] {
    const documents = [...this.characters.values()].map(({ document, best }) => {
      const dungeonRuns = [...best.values()].sort((left, right) => right.score - left.score);

      return {
        ...document,
        // Rounded to one decimal: scores arrive with one (515.3), and summing
        // eight floats produces trailing binary noise that would show up on a
        // board as 1841.2000000000003.
        mythicScore: Math.round(dungeonRuns.reduce((sum, run) => sum + run.score, 0) * 10) / 10,
